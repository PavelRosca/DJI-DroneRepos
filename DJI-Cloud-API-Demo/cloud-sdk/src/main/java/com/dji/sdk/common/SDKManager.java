...
public class SDKManager {
    ...
    public static GatewayManager registerDevice(String gatewaySn, String droneSn,
        ...
        try {
            gatewayType = GatewayTypeEnum.find(DeviceEnum.find(domain, type, subType));
        } catch (Exception e) {
            // Fallback for unknown/newer device types not yet in the SDK enums
                gatewayType = DeviceDomainEnum.DOCK == domain
                    ? GatewayTypeEnum.DOCK : GatewayTypeEnum.RC;
        }
        return registerDevice(gatewaySn, droneSn, gatewayType, gatewayThingVersion, droneThingVersion);
    }
